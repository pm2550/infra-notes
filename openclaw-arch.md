...
每次 Pi / WSL 完成任务后，OpenClaw 会根据结果自动 spawn 一个 Codex 子 agent，角色按情况切换：

- **任务成功 → 审查官模式**：质疑方法、验证结论、找漏洞。如果 Codex 发现问题，OpenClaw 回头 SSH 让 Pi/WSL 修，最多来回 2 轮。2 轮后还过不了就上报用户。
- **任务失败/卡住 → 鼓励官模式**：分析失败原因，提供新思路，给出下一步建议。OpenClaw 拿到建议后再试一次，如果跑通了切回审查官模式；还是不行就报给用户。

通用规则：同一个操作失败 2 次以上，直接停，不再重试，等用户指令。

这样代码结果不会直接入库，也不会无限循环卡死——Codex 兜底，兜不住就找人。
